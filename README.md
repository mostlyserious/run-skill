...
You use it when a task is too large, fuzzy, or high-stakes for a single prompt. Instead of throwing one giant instruction at an AI tool, `run` helps you shape the work into a clear execution package, then hands that package to a portable workflow runner that can execute, monitor, and resume it.
...
- `run-workflow --validate`: verify a package before launch
- `run-workflow --launch-mode ...`: execute the package
- `run-workflow --status`: print a one-shot status summary
- `run-workflow --follow`: stay attached to live progress
- `run-workflow --watch`: open a light supervision loop
- `run-workflow --dry-run`: inspect what would run without executing it

`run-skill` still works as a legacy compatibility alias, but `run-workflow` is the canonical command for new installs, docs, and scripts.
...
The `run` planner walks through the project with you, asks for clarification where it matters, and writes a run package to a folder like:
...
Once the package is approved, you use the workflow runner:

```bash
run-workflow --validate ./runs/<project>/blueprint.json
run-workflow --launch-mode standard ./runs/<project>/blueprint.json
```
...
- the `run` planner skill into `~/.codex/skills/` for Codex
- the `run` planner skill into `~/.claude/skills/` for Claude Code
- a Claude Code command wrapper at `~/.claude/commands/run.md`
- a `run-workflow` shim in `~/.local/bin`
- a `run-skill` compatibility shim in `~/.local/bin`
...
Restart Claude Code or Codex after install so the planner is discovered cleanly.
...
The planner will help you define:
...
`run-workflow` supports three main launch modes.
...
- `run-workflow --status` is the fastest way to inspect state
...
```bash
run-workflow --status ./runs/<project>/blueprint.json
```

To stay attached while it runs:

```bash
run-workflow --follow ./runs/<project>/blueprint.json
```

To watch the run with a lighter supervision surface:

```bash
run-workflow --watch ./runs/<project>/blueprint.json
```

## Resuming a Run

If a run stops, you can reopen it through the planner:
...
- the `run` planner is the planner
- `blueprint.json` is the contract
- `run-workflow` is the operator
